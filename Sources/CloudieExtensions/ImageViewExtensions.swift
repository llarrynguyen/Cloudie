...
/// them compatible with Cloudie image loading extensions.
///
/// The protocol is defined as `@objc` to make it possible to override its
/// methods in extensions (e.g. you can override `cloudie_display(image:data:)` in
...
@MainActor
@objc public protocol Cloudie_ImageDisplaying {
    /// Display a given image.
    @objc func cloudie_display(image: PlatformImage?, data: Data?)
...
extension Cloudie_ImageDisplaying {
    func display(_ container: ImageContainer) {
        cloudie_display(image: container.image, data: container.data)
    }
}
...
public typealias ImageDisplayingView = UIView & Cloudie_ImageDisplaying

extension UIImageView: Cloudie_ImageDisplaying {
    /// Displays an image.
    open func cloudie_display(image: UIImage?, data: Data? = nil) {
        self.image = image
    }
}
#elseif os(macOS)
...
public typealias ImageDisplayingView = NSObject & Cloudie_ImageDisplaying

extension NSImageView: Cloudie_ImageDisplaying {
    /// Displays an image.
    open func cloudie_display(image: NSImage?, data: Data? = nil) {
        self.image = image
    }
}
#endif
...
extension TVPosterView: Cloudie_ImageDisplaying {
    /// Displays an image.
    open func cloudie_display(image: UIImage?, data: Data? = nil) {
        self.image = image
    }
...
///   - view: Cloudie keeps a weak reference to the view. If the view is deallocated
    ...
    func loadImage(
        ...
        guard var request else {
            if options.isPrepareForReuseEnabled {
                imageView.cloudie_display(image: nil, data: nil)
            }
        ...
        if let placeholder = options.placeholder {
            ...
            imageView.cloudie_display(image: nil, data: nil) // Remove previously displayed images (if any)
        }
...
extension ImageViewController {
    ...
    private func runSimpleFadeIn(image: ImageContainer, params: ImageLoadingOptions.Transition.Parameters) {
        ...
        UIView.transition(
            ...
            animations: {
                imageView.cloudie_display(image: image.image, data: image.data)
            },
            completion: nil
        )
    }
